...
async function getTrain(trainID,start,end,startdate,enddate,starttime,endtime,decription){
    try {
        const trains = client.db('projectdb').collection('trains');
        console.log(trainID);
...
async function allSeats(trainID) {
    try {
        console.log(trainID);
        const seatss = client.db('projectdb').collection('seats');
        // console.log(trainID);
        const allSeats = await seats.findOne({trainID});
